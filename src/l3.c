...
int main(int argc, char ** argv){
  ...
  if(argc == 4){
    ...
    printf("lee_refined_filter_T3.c (modified by Ash Richardson)\nUsage: [Input directory] [Output directory] [Window Size]\n");
    exit(1);
  }
  ...
  for (lig = 0; lig < Sub_Nlig; lig++) {
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      ...
      for (l = -(Nwin - 1) / 2; l < 1 + (Nwin - 1) / 2; l++) {
        span[(Nwin - 1) / 2 + k][(Nwin - 1) / 2 + l] = M_in[T11][(Nwin - 1) / 2 + k][(Nwin - 1) / 2 + col + l] + M_in[T22][(Nwin - 1) / 2 + k][(Nwin - 1) / 2 + col + l] + M_in[T33][(Nwin - 1) / 2 + k][(Nwin - 1) / 2 + col + l];
      }
      ...
      for (k = 0; k < 3; k++) {
        for (l = 0; l < 3; l++) {
          ...
          subwin[k][l] += span[k * Deplct + kk][l * Deplct + ll] / (float) (Nnwin * Nnwin);
        }
      }
      ...
      for (k = -(Nwin - 1) / 2; k < 1 + (Nwin - 1) / 2; k++)
      for (l = -(Nwin - 1) / 2; l < 1 + (Nwin - 1) / 2; l++)
      if (Mask[Nmax][(Nwin - 1) / 2 + k][(Nwin - 1) / 2 + l] == 1) {
        for (Np = 0; Np < Npolar; Np++) mean[Np] += M_in[Np][(Nwin - 1) / 2 + k][(Nwin - 1) / 2 + col + l];
        m_span2 += span[(Nwin - 1) / 2 + k][(Nwin - 1) / 2 + l] * span[(Nwin - 1) / 2 + k][(Nwin - 1) / 2 + l];
        Npoints = Npoints + 1.;
      }

      /* Polarimetric coefficients averaging */
      for (Np = 0; Np < Npolar; Np++) mean[Np] /= Npoints;
      m_span = mean[T11] + mean[T22] + mean[T33];
      m_span2 /= Npoints;
      ...
      coeff = (cv_span * cv_span - sigma2) / (cv_span * cv_span * (1 + sigma2));
      if (coeff < 0.) coeff = 0.;

      /* Filtering f(x)=E(x)+k*(x-E(x)) */
      for (Np = 0; Np < Npolar; Np++)
      M_out[Np][col] = mean[Np] + coeff * (M_in[Np][(Nwin - 1) / 2][(Nwin - 1) / 2 + col] - mean[Np]);
    }
    /*col */
