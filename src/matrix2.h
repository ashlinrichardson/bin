...
template<class T> struct complex{
	...
	template<class T>
	inline ostream & operator<<( ostream &output, const complex<T> A){
		output.precision(5);
		output.fill(' ');
		output << right ;
		output << scientific;
	...
	template<class T>
	inline complex<T> conj(complex<T> const &A){
		complex<T> ret( A.re, - A.im );
		return ret;
	}

	template<class T>
	inline T real(complex<T> const &A){
		return A.re;
	}

	template<class T>
	inline T imag(complex<T> const &A){
		return A.im;
	}

	template<class T>
	inline T abs(complex<T> const &A){
	...
	template<class T>
	inline T arg( complex<T> const &A){
		return atan2( A.im , A.re);
	}

	template<class T>
	inline complex<T> polar(T r, T theta){
		complex<T> ret( r*cos(theta), r*sin(theta));
		return ret;
	}
	
	template<class T>
	inline complex<T> csqrt( complex<T> const &A){
		return polar( sqrt(abs(A)), arg(A)/2.);
	}

	template<class T>
	inline complex<T> cpow( complex<T> const &A, TYPE exponent){
		T r = abs(A);
		T theta = arg(A);
	...
	template<class T>
	inline complex<T> exp( complex<T> const &A){
	  T e = std::exp(real(A));
		return complex<T> (e*cos(A.im),e*sin(A.im));
	}

	template<class T>
	inline complex<T> cexp( complex<T> const &A){
    T e =std::exp( real(A) );
    return complex<T> (e*cos(A.im),e*sin(A.im));  
  }
	
	template<class T>
  inline void set( complex<T> &A, T imm, T ree){
		A.im = imm;
		A.re = ree;
  }
	...
	template<class T>
	inline complex<T>  operator*(const T &B,  const complex<T> &A){
		return complex<T> ( B* (A.re), B* (A.im));
	}
