/*adapted from Force_Fit_Application_Program/force_fit_v2_fast.cpp */
#include "string.h"
#include "stdlib.h"
#include "math.h"
#include <fstream>
...
int main(int argc, char ** argv){
  ...
  if(argc!=6){
    ...
    printf("Force fitting of hyperspectral data (ff.cpp) by Ash Richardson, updated April 29,2009.\n");
  ...
  char ch; /*Character buffer to read in comma separators*/

  /*Create arrays to store the input from the parameter file*/

  float * wave = new float[bands]; /*The band centre wavelengths*/
  memset(&wave[0], '\0', bands*sizeof(float));

  float * mul_coeff = new float[bands]; /*The multiplicative coefficients*/
  memset(&mul_coeff[0], '\0', bands*sizeof(float));

  float * add_coeff = new float[bands]; /*The additive coefficients*/
  ...
  out_file.write((char *)&(data_out[0]),samples*lines*bands*sizeof(float ));
  stored_time = clock();
