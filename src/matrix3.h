...
/* give vector l2 length of 1*/
  ...
  inline void normalize(vec3<T> & A){
		TYPE norm = sqrt(abs(A.a)*abs(A.a)+abs(A.b)*abs(A.b)+abs(A.c)*abs(A.c));
		(A.a)=(A.a)/norm; 
		(A.b)=(A.b)/norm; 
		(A.c)=(A.c)/norm;
	}
...
/* accessor */
  ...
  template<class T>
	inline vec3<T> conj(const vec3<T> & A){
...
struct herm3{
	...
	inline cf det(){
		cf retval = (cf)((a*d*f)-(a*e*conj(e))-(conj(b)*b*f)+(conj(b*e)*c)+(conj(c)*b*e)-(conj(c)*c*d));
		if( imag(retval) > 0.0000001){
...
/* frobenius norm */
 ...
 /* sum of positive definite matrices is positive definite: */
	inline TYPE d3( const herm3<T> & Z){
		if(real( det()) <  0){
			printf("Error: determinant of hermitian matrix was negative.\n");
			cout << *this <<endl;
			cout << det() <<endl;
			exit(1);
		}
		
		/* Eqn. 8.13, p.268 of "POLARIMETRIC RADAR IMAGING"
		  Assume this object is the class centre, Cm
		  d3(Z, Cm) = ln(det(Cm)) + trace( inv(Cm)*Z )        */
		cf val = (-Z.a*d*f+Z.a*e*conj(e)+conj(Z.b)*b*f-conj(Z.b)*c*conj(e)-conj(Z.c)*b*e+conj(Z.c)*c*d+Z.b*conj(b)*f-Z.b*e*conj(c)-Z.d*a*f
                 	+Z.d*c*conj(c)+conj(Z.e)*a*e-conj(Z.e)*c*conj(b)-Z.c*conj(b)*conj(e)+Z.c*d*conj(c)+Z.e*a*conj(e)-Z.e*b*conj(c)-Z.f*a*d+Z.f*b*conj(b)
                	)/(-a*d*f+a*e*conj(e)+conj(b)*b*f-conj(b)*c*conj(e)-conj(c)*b*e+conj(c)*c*d);
		if( imag(val) > 0.000001){
			printf("Warning: in formula d3() the trace should have no imaginary part.\n");
		}

		TYPE retval = real((det()).log())  + real(val);
		return retval;
	}

	inline TYPE Dij(herm3<T> & C){
		/* equation 8.26 of p271 of "POLARIMETRIC RADAR IMAGING"*/

		TYPE a1 = log(real((det()))) ;
		TYPE a2 = log(real((C.det()))) ;
		if(imag(det()) >0.00000001){
			printf("Error: unreal determinant\n");
			exit(1);
		} 

		if(imag(C.det()) >0.00000001){
			printf("Error: unreal determinant\n");
			exit(1);
		} 

		cf a3 = ((inv() * C) +    ((C.inv())*(*this)) ).trace();
		if(imag(a3) > 0.000000001){
			printf("Error: trace should be real valued.\n");
			exit(1);
		}
		TYPE retval = 0.5*(a1+a2+real(a3));
		return retval;
	}
	...
	template<class T>
	inline herm3<T> operator * (double s, const herm3<T> & A){
		 return herm3<T>( s*(A.a), s*(A.b), s*(A.c), s*(A.d), s*(A.e), s*(A.f));		
	}

	template<class T> 
  inline ostream & operator<<( ostream &output, const herm3<T> A){
  ...
  inline matrix3<T> operator + ( const matrix3<T> & A, const matrix3<T> & B){
	...
	template<class T>
	inline matrix3<T> operator * ( const matrix3<T> & X, const herm3<T> & A){		
		return matrix3<T>( 
...
/* add case to avoid div by 0 */
  inline vec3<cf> solve_cubic(cf a, cf b, cf c, cf d){
    ...
    cf t0 =  (t1 + cpow(    4.*(t2*t2*t2) +  (t1*t1) ,0.5));

    cf t3  = cpow( t0   , 0.333333333333333333333333)     ;
