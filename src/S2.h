//;***********************************************************************************************************
//;*
//;* S2.h formerly squall\palsar\pos\RADLIB\S2.h
//;*
//;***********************************************************************************************************
//;*
//;* This header performs library type functions for S2 matrix data. See:
//;* squall/wiki/index.php/RADLIB
//;*
//;* Status: R&D
//;*
//;* Last modification:March 18, 2008.
//;*
//;* Developer: Ashlin Richardson <ashricha@nrcan.gc.ca> <ashlin.richardson@gmail.com>
//;*
//;* Modified by A. Richardson on March 18th, 2008 for commenting. 20170510 revisions
//;*
//;***********************************************************************************************************
...
namespace _S2{
	class S2{ //Class representing an S2 data set, for the reading and writing of S2 datasets.
		...
		public:
			...
			void init_S2(int _type, char * _dir, char * fn[]){	
				...
				for (i=0; i<nS2Files; i++){
					 //printf("pixeli %d", i);
					 (pixel[i]).zero(); //=0.0;  //initialize the buffer pixel to be zero.
				}
				...
				//NB output S2 data sets require the dimensions to be set.
				for (i=0; i<nS2Files; i++){	
					//Open all the corresponding files in the input directory.
					sprintf(file_name, "%s%s", dir, filenames[i]);
					S[i].initImage(_type, file_name);
					
					//For each channel, set the dimensions.
					if(type==INPUT) S[i].setDimensions(NRow, NCol);
					file_name[0]='\n';
			...
			void setPixel()  //Store the buffer pixel to file.
			{
				for(i=0; i<nS2Files; i++){
			...
			void getPixel()  //Fetch a pixel from the file, into the buffer pixel.
			{
				if(!dimensions_set)
				{
			...
			void close()  //Close the S2 file object by closing the individual channels (complex images)
			{
				for(i=0; i<nS2Files; i++) S[i].close();
			}
			...
			void setDimensions(int _NRow, int _NCol){
				...
				for(i=0; i<nS2Files; i++) S[i].setDimensions(NRow, NCol);
			}
	};
};
