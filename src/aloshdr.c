...
* *
* Description: This stand-alone task reads CEOS formatted ALOS-PALSAR files *
* from disk and creates ASCII readable files of the contents. *
* *
* Input Files: The following CEOS formatted files are read by this task: *
* *
* leader file *
* image file *
* trailer file *
...
int main(int argc, char *argv[]) {
  ...
  if (argc == 6) {
    ...
    strcpy(leader_in, argv[2]);
    strcpy(image_in, argv[3]);
    strcpy(trailer_in, argv[4]);
    strcpy(PSPConfigFile, argv[5]);
  }
  else {
    printf("TYPE: alos_header DirOutput LeaderFile ImageFile TrailerFile ConfigFile\n");
    exit(1);
  }
...
void Rd_SAR_Trailer(){
  ...
  if ((nread = fread(&trailer, sizeof(trailer_file_struct), 1, fp_rd)) < 1){
    ...
    databmp = matrix_float(Nlig, Ncol);
    bufferdata = vector_char(2*Ncol);

    // strcpy(ColorMap,"Config/ColorMapGRAY.pal");
    // check_file(ColorMap);
    ...
    for (lig = 0; lig < Nlig; lig++) {
      ...
      for (col = 0; col < Ncol; col++) {
        ...
        databmp[lig][col] = (float) (256. * LS + MS);
    ...
    for (lig = 0; lig < Nlig; lig++) {
      for (col = 0; col < Ncol; col++) {
        ...
        databmp[lig][col] = 255. * xx;
      }
    }
    // bmp_8bit(Nlig, Ncol, Max, Min, ColorMap, databmp, lowresimg_ascii);
  }
...
*
* Description: This routine re-arrages the byte-order for VAX/VMS short
* word integer
*
* Include files: none
*
